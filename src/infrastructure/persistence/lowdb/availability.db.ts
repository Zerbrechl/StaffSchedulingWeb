...
export async function getAvailabilityDb(caseId: number, monthYear: string) {
    const [month, year] = monthYear.split('_').map(Number);
    const fromDate = new Date(Date.UTC(year, month - 1, 1));
    const url = new URL(`${getSolverApiConfig().baseUrl}/availability`);
    ...
    if (response.ok) {
        ...
        if (!parsed.success) {
            throw new Error('Invalid availability response from backend API');
        }
    ...
    return {
        ...
        async write() {
            ...
            if (!response.ok) {
                ...
                throw new Error(message || 'Failed to write availability to backend API');
            }
        },
    };
}
